...
class persona_rag:
    def __init__(self):
        ...
        self.wiki_text_db = duckdb.connect("wiki_corp.duckdb")
        ...
        self.string_format_agent = Agent(name = "Json Formatting Agent", instructions=f"Because LLM outputs can be inconsistent, your role is to adjust the output such that json.loads() can parse LLM outputs.")
    ...
    def ask_question(self, query):
        ...
        cot_prompt = f"Question: {query}, Passages: {retr_ranked_articles}, 1. Read the given question and passages to gather relevant information. 2. Write reading notes summarizing the key points from these passages. 3. Discuss the relevance of the given question and passages. 4. If some passages are relevant to the given question, provide a brief answer based on the passages. 5. If and only if no passage is relevant at all, just state \"no answer found\""
